...
class FacturaController extends Controller
{
    ...
    public function edit($id)
    {
        ...
        $factura = Factulinea::where('id_formula', $id)->first();

        //$factura = Factulinea::find($id);
        $clientes = Cliente::all();
        $tipos_facturacion = TipoFacturacion::all();
        $productos = Producto::all();

        if ($factura !== null) {
            $facturapro = $factura->id_producto;
            // Other operations with $producto
        } else {
            $facturapro = 'selected';
        }
        //dd($factura);

        return view('facturas.edit', compact('factura', 'clientes', 'tipos_facturacion', 'productos', 'facturapro'));
    }
